...
- **Rich Text Notes + AI** — Full Quill editor with six section chips that trigger AI-generated content (travel tips, packing list, transit guides, etc.) via Google Gemini. Shared across all trip members. Opens in read mode with an explicit edit toggle; unsaved edits are kept as a local draft (restored on return, with an unsaved-changes indicator) so switching tabs never loses work. The toolbar covers headings, colours, links, lists, quotes, collapsible blocks, dividers and tables — tables are inserted by picking a size from a grid, and a row/column toolbar appears whenever the cursor sits inside one. A collapsible block takes the first line as its heading and tucks everything below it inside; it stays open while editing so the content is always reachable, and reading mode opens it folded, so a long note can be skimmed by its headings and unfolded only where needed. Pressing Enter on an empty last line steps back out of the block, and backspacing at the start of its heading turns the whole thing back into ordinary paragraphs. Shift+Enter starts a new line inside the current block, so a quote, list item or paragraph can span several lines, while Enter still starts a new one. Reading mode uses the same typography as the editor, so a note looks identical before and after saving. The same editor powers itinerary notes and trip descriptions.
...
- **筆記分頁（富文字 + AI）** — 完整 Quill 富文字編輯器，六個區塊 Chip 可觸發 AI 生成旅遊內容（旅遊注意事項、該帶什麼、地鐵攻略等），由 Google Gemini 驅動，所有成員共享。預設為閱讀模式、點「編輯」才進入編輯器；未儲存的編輯會自動存成本機草稿（回來時還原並提示），切換分頁不再遺失內容。工具列涵蓋標題、顏色、連結、清單、引言、折疊區塊、分隔線與表格；表格以格線選擇尺寸後插入，游標移入表格時會展開增減列／欄與刪除表格的操作列。折疊區塊會把第一行當成標題、其餘內容收在底下，編輯時保持展開隨時能改，閱讀模式則預設收起來，長筆記可以先看標題再點開需要的那一段。在最後一行空行按 Enter 會跳出折疊區塊，在標題最前面按 Backspace 則整塊還原成一般段落。Shift+Enter 會在同一個區塊內換行，引言、清單項目或段落都能寫成多行，Enter 則照常另起新的一段。閱讀模式與編輯模式使用同一套排版，存檔前後看到的版面完全一致。行程備註與旅程簡介也使用同一個編輯器。
